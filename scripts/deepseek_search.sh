...
EOF
    ...
    log "Search submitted. Determining wait time based on query complexity..."
    
    # Determine wait time based on query complexity
    # Simple queries: short, factual questions (wait 5 seconds)
    # Complex queries: long, analytical questions, or those needing web search (wait 15 seconds)
    if [[ ${#QUERY} -lt 20 ]] && [[ ! "$QUERY" =~ (分析|比较|总结|最近|新闻|研究|探讨|如何|为什么|什么原因|解决方案|建议|推荐|优化) ]]; then
        WAIT_TIME=5
        log "Simple query detected (${#QUERY} chars). Waiting 5 seconds..."
    else
        WAIT_TIME=15
        log "Complex query detected (${#QUERY} chars). Waiting 15 seconds..."
    fi

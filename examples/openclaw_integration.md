...
This document explains how to use the DeepSeek Search Skill within OpenClaw agent sessions.

## Skill Activation

The skill will be automatically activated when OpenClaw detects keywords related to DeepSeek search:
...
**User**: "搜索deepseek获取广州旅游景点推荐"

**Agent Response**:
```
我将使用DeepSeek搜索技能帮你获取广州旅游景点推荐。

请按照以下步骤操作：
1. 确保Chrome浏览器已打开，并访问DeepSeek页面
2. 手动输入搜索词："广州旅游景点推荐"
3. 按Enter键等待DeepSeek生成结果
4. 完成后告诉我，我将提取搜索结果
...
**User**: "我需要从deepseek获取2026年2月份广州十大游玩地，然后整理成行程表"

**Agent Workflow**:
1. Activate DeepSeek Search skill
...
def handle_deepseek_search(query):
    """
    Handle DeepSeek search request.
    """
    # Step 1: Instructions for manual search
    instructions = f"""
我将帮你从DeepSeek搜索: "{query}"

**请手动完成以下步骤:**
1. 打开Chrome浏览器 (如果未打开)
2. 访问DeepSeek页面
3. 输入搜索词: "{query}"
4. 按Enter键等待DeepSeek生成结果
...
def extract_deepseek_results():
    ...
    skill_dir = os.path.expanduser("~/.openclaw/workspace/skills/deepseek-search")
    script_path = os.path.join(skill_dir, "scripts", "deepseek_search.sh")
    
    if not os.path.exists(script_path):
        return "错误: DeepSeek搜索技能未安装"
    ...
    try:
        ...
        return "错误: 提取超时，请检查DeepSeek页面是否正常"
...
class DeepSeekSearchIntegration:
    """Integration class for DeepSeek Search skill."""
    
    def __init__(self, skill_path: str = None):
        self.skill_path = skill_path or os.path.expanduser("~/.openclaw/workspace/skills/deepseek-search")
        self.script_path = f"{self.skill_path}/scripts/deepseek_search.sh"
    ...
    def get_search_instructions(self, query: str) -> Dict[str, str]:
        ...
        return {
            ...
            "notes": [
                "如果页面需要登录，请先登录DeepSeek账号",
                "确保网络连接正常",
                "如果DeepSeek响应慢，请等待10-20秒"
            ]
        }
    
    def extract_results(self, verbose: bool = False) -> Tuple[bool, str]:
        """Extract results from DeepSeek page."""
        import subprocess
        ...
        try:
            ...
            return False, "提取超时，请检查DeepSeek页面"
        except Exception as e:
...
### Step 1: User Request
**User**: "帮我从deepseek搜索'Python学习路线图'"

### Step 2: Agent Response
```
我将使用DeepSeek搜索技能帮你获取Python学习路线图。
...
integration = DeepSeekSearchIntegration()
...
1. DeepSeek还未生成完整回答（请等待10秒）
2. 搜索词未正确输入
3. 页面内容结构有变化

请确认已看到DeepSeek的回答，然后重试。
"""
```
...
```
User: 我想知道从deepseek获取广州美食推荐
Agent: 我将帮你从DeepSeek搜索广州美食推荐。请先手动操作...
User: 已完成搜索
...
skills:
  deepseek-search:
    enabled: true
    path: ~/.openclaw/workspace/skills/deepseek-search
    auto_activate: true
    activation_keywords:
      - "deepseek"
      - "DeepSeek"
...
from examples.openclaw_integration import DeepSeekSearchIntegration
...
4. **Concurrency**: Only one DeepSeek search at a time (browser limitation)